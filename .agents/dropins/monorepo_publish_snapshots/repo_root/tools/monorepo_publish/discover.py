    ...
    for child in sorted(packages_dir.iterdir()):
        ...
        if not isinstance(project, dict):
            ...
            if isinstance(tool, dict) and isinstance(tool.get("poetry"), dict):
                raise DiscoverError(
                    "Unsupported pyproject format (Poetry). "
                    f"{pyproject_path} uses [tool.poetry] metadata, but this publisher requires "
                    "PEP 621 metadata under [project]."
                )
            raise DiscoverError(f"Missing/invalid [project] table in {pyproject_path} (expected PEP 621 metadata).")
