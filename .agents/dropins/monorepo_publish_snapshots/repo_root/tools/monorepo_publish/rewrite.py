...
def rewrite_pyproject_for_snapshot(pyproject_path: Path, package_versions: dict[str, str]) -> None:
    ...
    if deps is not None:
        ...
        project["dependencies"] = [
            _rewrite_req(str(req_s), package_versions=package_versions, self_name=self_key) for req_s in deps
        ]
    ...
    if opt is not None:
        ...
        for group, group_deps in list(opt.items()):
            ...
            opt[group] = [
                _rewrite_req(str(req_s), package_versions=package_versions, self_name=self_key) for req_s in group_deps
            ]
